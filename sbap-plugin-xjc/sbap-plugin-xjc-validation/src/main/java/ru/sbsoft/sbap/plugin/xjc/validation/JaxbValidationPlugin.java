...
public class JaxbValidationPlugin extends Plugin {
    ...
    private void processNotNull(ClassOutline co, JFieldVar field) {
        if (notNullAnnotations) {
            ...
            co.target.getProperty(field.name()).getCustomizations().stream().forEach(e -> {
                final Element element = e.element;
                try {
                    final Node valueNode = (Node) X_PATH.evaluate("/*[local-name()='emptyError']/*[local-name()='message']/@value", element, XPathConstants.NODE);
                    if (valueNode != null) {
                        final String value = valueNode.getTextContent();
                        if (value != null) {
                            annotation.param("message", value);
                        }
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
    ...
    public void processType(XSSimpleType simpleType, JFieldVar field, String propertyName, String className) {
        if (!hasAnnotation(field, Size.class) && isSizeAnnotationApplicable(field)) {
            final Integer maxLength = simpleType.getFacet("maxLength") == null ? null : Utils.parseInt(simpleType.getFacet(
                    "maxLength").getValue().value);
            final Integer minLength = simpleType.getFacet("minLength") == null ? null : Utils.parseInt(simpleType.getFacet(
                    "minLength").getValue().value);
            if (minLength != null) {
                log("@Min(" + minLength + "): " + propertyName + " added to class " + className);
                final JAnnotationUse au = field.annotate(Min.class).param("value", minLength);
                analizeRestrictionMessage(au, simpleType.getFacet("minLength"));
            }
            if (maxLength != null) {
                log("@Max(" + maxLength + "): " + propertyName + " added to class " + className);
                final JAnnotationUse au = field.annotate(Max.class).param("value", minLength);
                analizeRestrictionMessage(au, simpleType.getFacet("maxLength"));
            }
        }
        ...
        if (maxInclusive != null && Utils.isNumber(field) && isValidValue(maxInclusive)
            ...
            final JAnnotationUse au = field.annotate(DecimalMax.class).param("value", maxInclusive.getValue().value);
            analizeRestrictionMessage(au, maxInclusive);
        }
        ...
        if (minInclusive != null && Utils.isNumber(field) && isValidValue(minInclusive)
            ...
            log("@DecimalMin(" + minInclusive.getValue().value + "): " + propertyName
                    + " added to class " + className);
            final JAnnotationUse au = field.annotate(DecimalMin.class).param("value", minInclusive.getValue().value);
            analizeRestrictionMessage(au, minInclusive);
        }
        ...
        if (maxExclusive != null && Utils.isNumber(field) && isValidValue(maxExclusive)
            ...
            analizeRestrictionMessage(annotate, maxExclusive);
        }
        ...
        if (minExclusive != null && Utils.isNumber(field) && isValidValue(minExclusive)
            ...
            analizeRestrictionMessage(annotate, minExclusive);
        }
        ...
        if (simpleType.getFacet("totalDigits") != null && Utils.isNumber(field)) {
            ...
            if (!hasAnnotation(field, Digits.class)) {
                ...
                analizeRestrictionMessage(annox, minExclusive);
            }
        }
        /**
        ...
        if (patternList.size() > 1) { // More than one pattern
            ...
            if ("String".equals(field.type().name())) {
                ...
                if (!"\\c+".equals(pattern)) {
                    ...
                    analizeRestrictionMessage(getAnnotation(field, Pattern.class), simpleType.getFacet("pattern"));
                }
            }
    ...
    private void analizeRestrictionMessage(JAnnotationUse validationAnnotation, XSFacet restriction) {
        final XSAnnotation xSAnnotation = restriction.getAnnotation();
        if (xSAnnotation == null) {
            return;
        }
        final Iterable<BIDeclaration> annotation = (Iterable<BIDeclaration>) xSAnnotation.getAnnotation();
        if (annotation == null) {
            return;
        }

        annotation.iterator().forEachRemaining(decl -> {
            final Element element = ((BIXPluginCustomization) decl).element;
            try {
                final Node valueNode = (Node) X_PATH.evaluate("/*[local-name()='formatError']/*[local-name()='constant']/@value", element, XPathConstants.NODE);
                final String value = valueNode.getTextContent();
                if (value != null) {
                    validationAnnotation.param("message", value);
                }
...
//		}
    ...
    public JAnnotationUse getAnnotation(JFieldVar var, Class annotationClass) {
        return ((List<JAnnotationUse>) Utils.getField("annotations", var))
                .stream().filter(au -> ((Class) Utils.getField("clazz._class", au)).getCanonicalName().equals(annotationClass.getCanonicalName()))
                .findFirst().get();
    }
